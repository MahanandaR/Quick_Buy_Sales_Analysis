...
# Orders DataFrame
orders_cols = ["order_id", "order_date", "customer_id", "product_id", "ship_mode", "ship_date", "sales", "quantity",
               "discount", "profit"]
...
def random_orders_data(lower_limit, upper_limit):
    ...
    sales_weight = pd.cut(orders["sales"], bins=[0, 500, 1000, 2000, int(orders["sales"].max())]).value_counts(
